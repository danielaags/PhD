...
%enough
%stack, adquisition treated with gaussian filter 3 pixels
%time, adquisition time
%off, offset to take. Default 1 

clear all;

close all;

stack='3ul_ftsz-mts_10min03_R3D-1_mean3.tif';
proj_stack='AVG_3ul_ftsz-mts_10min03_R3D-1_mean3.tif';
time=3;
off=1;
%offset=15;
DistancePix= 1.5*9.3721;
...
r=sqrt((x0(2*j-1)-x0(2*j))^2+(y0(2*j-1)-y0(2*j))^2);

B=Im(x0(2*j-1)-r:x0(2*j-1)+r, y0(2*j-1)-r:y0(2*j-1)+r);
...
%level = graythresh(kymograp);
% kymograp = im2bw(kymograp,0.2);
%level = graythresh(kymograp2);
% kymograp2 = im2bw(kymograp2,0.2);
%level = graythresh(kymograp3);
% kymograp3 = im2bw(kymograp3,0.2);

radii(1)=r;


%kymograp = adapthisteq(kymograp);
%kymograp2 =adapthisteq(kymograp2);
%kymograp3 = adapthisteq(kymograp3);
l=size(tsStack,2)+7;
%Save kymograph
figure
...
set(tx, 'FontSize', 10);
print(strcat(s,'kymo_meanV'),'-dpng');
close;

T=table(xunit_1, yunit_1, radii', vel');
writetable(T,strcat(s,'kymo_mean.txt'),'Delimiter','\t');
else


close;
T=table([xunit_1 yunit_1 radii']);
writetable(T,strcat(s,'kymo_mean.txt'),'Delimiter','\t');
end
